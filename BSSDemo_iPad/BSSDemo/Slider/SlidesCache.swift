...
class SlidesCache {
    ...
    init?(fileName : String) {
        ...
        if NSFileManager.defaultManager().fileExistsAtPath(self.fileName.appSliderPathObjects.cachePath) {
            do{
                let jsonStr = try String(contentsOfFile: self.fileName.appSliderPathObjects.cachePath, encoding: self.stringFileEncoding)
    ...
    func createCache() {
        ...
        do{
            ...
            try jsonStr?.writeToFile(fileName.appSliderPathObjects.cachePath, atomically: true, encoding: stringFileEncoding)
        } catch let error as NSError {
            print(error)
        }
    }
    ...
    func updatePDFCache(){
        if let pdf =  fileName.appSliderPathObjects.PDF {
            numberOfPages = pdf.numberOfPages
            thumbnail =  pdf.getPageImage(1, width: SliderGlobalVar.thumbnailSize.width)
            createCache()
        }
    }
}
