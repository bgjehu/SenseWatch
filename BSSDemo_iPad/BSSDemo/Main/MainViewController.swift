...
class MainViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBOutlet var bandSensorSwitch: UISwitch!
    ...
    @IBAction func bandSensorSwitchValueChanged(sender: UISwitch) {
        if sender.on {
            (UIApplication.sharedApplication().delegate as! AppDelegate).bandSensorManager.startClassification()
        } else {
            (UIApplication.sharedApplication().delegate as! AppDelegate).bandSensorManager.stopClassification()
        }
    }
    ...
    var selectedAction = 0 {
        didSet{
            ...
            descriptionTextView.textAlignment = description == "" ? NSTextAlignment.Center : NSTextAlignment.Left
            descriptionTextView.text = description == "" ? "No Description" : description
            submitButton.setTitle(getActionAvailability(selectedAction) ? "Demo" : "Sign Up", forState: UIControlState.Normal)
        }
    }
    ...
    func enterDemo(storyboardName : String){
        if storyboardName == "" {
            ...
            let controller = storyboard.instantiateViewControllerWithIdentifier("SignUp")
            controller.modalTransitionStyle = UIModalTransitionStyle.CrossDissolve
            self.presentViewController(controller, animated: true, completion: nil)
        } else if storyboardName == "Sphero" {
            UIApplication.sharedApplication().openURL(NSURL(string: "swsd://")!)
        } else {
    ...
    private func adjustUIForDevices() {
        ...
        descriptionTextView.font = UIFont.systemFontOfSize(20)
    ...
    func handleBandConnection() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "unlockUI", name: "MSBAND_NOTIFICATION_CONNECTED", object: nil)
    }
}
